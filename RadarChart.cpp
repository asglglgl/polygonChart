...
void RadarChart::setPointType(PointType pointstype)
{
...
void RadarChart::init()
{
	...
	m_seriesColor = QColor(152, 220, 238);
	m_labelFont = QFont("Arial", 9);
	m_backgroundColor = Qt::white;

}

void RadarChart::paintEvent(QPaintEvent *e)
{
...
void RadarChart::setTitle(const QVector<QString> &titile)
{
    m_titile.clear();
    for(auto one :titile )
    {
...
void RadarChart::drawAngularLabel(QPainter *painter)
{
	...
	for (int i = 0; i < m_AngularCount; ++i)
	{
        ...
        strWidth = fontMetrics.width(m_titile.at(i));
        ...
        painter->drawText(cur_x, cur_y, m_titile.at(i));
	}
	painter->restore();
}
...
//            qreal valueLength = m_datas[0].at(i).value * step+i*10;
...
void RadarChart::drawLinePoints(QPainter *painter)
{
    ...
    for(int j=0;j<m_datas.size();++j)
    {
        ...
        painter->setPen(m_datas[j].color);
        painter->setBrush(m_datas[j].color);
        for (int i = 0; i < m_AngularCount; i++)
        {


            qreal valueLength = m_datas[j].value.at(i) * step;
            cur_x = x0 + valueLength*qSin(dRadian*i);
            cur_y = y0 - valueLength*qCos(dRadian*i);
            points[i] = QPointF(cur_x, cur_y);
